...
void bernoulli_gen(int v[], int bernoulli_percentage, int n) {
    for(int i = 0; i < n; i++)
        v[i] = rand() % 100 < bernoulli_percentage;
}
...
void calculate_cdf(double cdf[], int trials) {
    ...
    for(int i = 0; i < TWO_DICE_SUM; i++) 
        cdf[i] = cdf[i] * 100 / cdf[TWO_DICE_SUM - 1];
}
...
void monty_hall(int *wins, double *win_probability, int n) {
	*wins = 0;
    for(int i = 0; i < n; i++) {
		...
		// int remaining = price == choice; // your second choice (if you switch)
		if (prize != choice) ++(*wins); // you win if you switched
    }
