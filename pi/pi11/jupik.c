...
void dump_list( const List* p_list ) {
	ListElement *roll = p_list->head;

	while ( roll ) {
		if ( p_list->dump_data ){
			p_list->dump_data( roll->data );
		}
		roll = roll->next;
	}
}
...
void dump_list_if ( List *p_list, void *data ) {
	...
	while ( roll ) {
		if ( p_list->dump_data && p_list->compare_data( data, roll->data) == 0){
			p_list->dump_data( roll->data );
		}
		roll = roll->next;
	}
...
void free_list( List* p_list ) {
	...
	ListElement *roll = NULL;
	roll = p_list->head;

	while ( roll != NULL ) {
		ListElement *to_remove = roll;
		roll = roll->next;
		if ( p_list->free_data ) 
			p_list->free_data( 
				to_remove->data 
